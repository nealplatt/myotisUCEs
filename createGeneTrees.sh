...
#################################################################################
# setting up bootstrapping files.
#################################################################################
WORK_DIR=/home/roplatt/myotisUCEs/neal/play

cd $WORK_DIR/15_geneTrees_raxml
#create a new directory containing all of the bootstrap trees per missing data
for i in 15 25 35 45 55 65 75 85 95 100
do

    mkdir $WORK_DIR/$i"_bootTrees"

    for UCE in $(ls $WORK_DIR/$i"_alignments"/ | sed 's/.nexus//')
        do
        cp "RAxML_bootstrap."$UCE"-rBoot" $WORK_DIR/$i"_bootTrees"
        done
